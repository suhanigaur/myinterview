...
function QuestionsSections({ activeQuestionIndex, mockInterViewQuestion }) {
  ...
  // Ensure mockInterViewQuestion is an array and not empty
  if (!Array.isArray(mockInterViewQuestion) || mockInterViewQuestion.length === 0) {
    return <p>No questions available.</p>; // Optionally return a fallback message or component if no questions
  }
    ...
    <div className='p-5 border rounded-lg my-10'>
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 text-center'>
            {mockInterViewQuestion.map((question, index) => (
                 <h2 key={index + 1} className={`p-2 bg-secondary rounded-full text-xs md:text-sm text-center cursor-pointer ${activeQuestionIndex === index && '!bg-primary text-white'}`}>
                    Question #{index + 1}
                 </h2>
            ))}
        </div>
